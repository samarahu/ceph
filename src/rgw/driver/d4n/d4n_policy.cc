...
// in the Cleaning method and prevents data races during concurrent requests. This method returns "false"
// if the state has not been set to INVALID, and "true" if it has.
bool LFUDAPolicy::invalidate_dirty_object(const DoutPrefixProvider* dpp, std::string& key) {
  ...
  if (p->second.second == State::INIT) {
    ...
    int ret = cacheDriver->set_attr(dpp, "D#" + key, RGW_CACHE_ATTR_INVALID, "1", y);
    if (ret < 0) {
    ...
    state_cond.wait(l, [this, &p]{ return (p->second.second == State::DONE); });
  }
...
int LFUDAPolicy::delete_data_blocks(const DoutPrefixProvider* dpp, LFUDAObjEntry* e, optional_yield y) {
  ...
  do {
    ...
    ldpp_dout(dpp, 0) << __func__ << "(): Sam: " << prefix << dendl;
...
void LFUDAPolicy::cleaning(const DoutPrefixProvider* dpp)
{
  ...
  while(!quit) {
	...
	int ret = -1;
	if ((ret = cacheDriver->delete_data(dpp, "D#" + e->key, y)) == 0) { // Sam: do we want del or delete_data here?
	...
	if (ret == 0) {
      ...
      if (!invalid && (std::difftime(time(NULL), e->creationTime) > interval)) { // if block is dirty and written more than interval seconds ago
	l.lock();
