...
{
  bufferlist bl_val;
  bl_val.append(std::to_string(this->get_obj_size()));
...
ldpp_dout(save_dpp, 10) << "AMIN::" << __func__ << "(): " << __LINE__ << " prefix is: "  << prefix << dendl;
	...
	ret = next->process(std::move(data), offset);
        ...
        if (ret == 0) {
          driver->get_policy_driver()->get_cache_policy()->update(save_dpp, oid, ofs, bl.length(), version, dirty, creationTime, std::get<rgw_user>(obj->get_bucket()->get_owner()), y);
