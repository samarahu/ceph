...
class LFUDAPolicyFixture : public ::testing::Test {
  protected:
	...
	if (exists > 0) { /* Remote copy */
	  ...
	  block->hostsList.insert(env->dpp->get_cct()->_conf->rgw_local_cache_address);
...
TEST_F(LFUDAPolicyFixture, LocalGetBlockYield)
{
  spawn::spawn(io, [this] (spawn::yield_context yield) {
    ...
    policyDriver->get_cache_policy()->update(env->dpp, key, 0, bl.length(), "", false, optional_yield{io, yield});
...
TEST_F(LFUDAPolicyFixture, RemoteGetBlockYield)
{
  spawn::spawn(io, [this] (spawn::yield_context yield) {
    ...
    policyDriver->get_cache_policy()->update(env->dpp, victimKey, 0, bl.length(), "", false, optional_yield{io, yield});
