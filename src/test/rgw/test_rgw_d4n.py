...
def test_small_object(r, client, obj):
    ...
    out = subprocess.check_output(['ls', '/tmp/rgw_d4n_datacache']).decode('latin-1') 
    assert(out.splitlines()[0] == "bkt_test.txt_0_4")
    out = subprocess.check_output(['ls', '/tmp/rgw_d4n_datacache_c2']).decode('latin-1') 
    assert(out.splitlines()[0] == "D_bkt_test.txt_0_4")
    ...
    out = subprocess.check_output(['cat', '/tmp/rgw_d4n_datacache_c2/D_bkt_test.txt_0_4']).decode('latin-1') 
    assert(out == "test")
